...
#table(
  ...
  [OSI 模型], [TCP/IP 模型], [功能],
  [应用层], table.cell(rowspan: 3)[应用层], [应用],
  [表示层], [数据表示, 格式转换, 加解密],
...
ISOC 是互联网协会, 下辖
- IAB: 互联网结构委员会
- IETF: 制定互联网标准(STD文档, RFC(Request for Comment)文档)的, 解决实际的*工程*问题
- IRTF: 互联网技术*研究*
- 等
...
$
  (24 times 7)/(24 times 8 + 1) approx 87.05%
$
...
802.11 的帧格式见 #link(<sect-pdu-struc>)[各协议 PDU 及相关数值].
...
#table(
  ...
  [环回地址],
  [127.\*],
  [用于本机测试, 注意 127.0.0.0 不行, 因为是网络地址. 此外, 环回地址不会发送到网络上, 直接在本机处理, 所以不做源地址],
)
...
+ 原MAC地址为 `AA-BB-CC-DD-EE-FF`
+ 在正中间插入 `FFFE`: `AA-BB-CC-FF-FE-DD-EE-FF`
+ 翻转U/L位(最高一组的次低位): `A8-BB-CC-FF-FE-DD-EE-FF`
...
- 构造 LSA(链路状态公告): 一个小地图, 包括*邻居*信息, 到邻居的链路, 链路上的量度
...
全毗邻关系的建立过程和状态迁移:
+ 互发 Hello (Down Init 初始状态), 建立双向双边关系 (Two-way 双向状态)
+ 首次交换 DD, 确立主从关系, 交换初始序列号 (ExStart 准启动状态)
+ 交换全部 DD (Exchange 交换状态)
...
+ 确认机制: 确认号是期望收到的下一个数据段的序号, 也即已经成功接收的数据号的后继. TCP 使用累计确认, 如果确认了后来的数据, 表示前面的数据也已经成功接收. 数据中可以携带确认号, 这叫做捎带确认(piggybacking).
+ 重传机制
  + 计时器超时: 太长时间没收到确认就重传, 超时时间设置为预估的往返时间
  + 三次重复确认: 如果中间一个丢失了, 后面的正常发送, 于是对于后面正常接收的每一个, 都会回复一个丢失包的序号的确认, 三次重复确认就会使发送方重传. 注意是三次*重复*, 所以事实上应该有四次都是一样的 ACK.
...
- *快速重传*和恢复阶段: 如果只有个别数据丢失(比如三次重复确认), 就立即重传, 把阈值设置为窗口的一半, 窗口设置为新的阈值, 然后进入拥塞避免阶段
  ...
  auto-div-frame(theme => {
    stack(dir: ttb, box(height: 1.5em, info), ..body
      ...
      .map(x => stack(dir: ltr, ..x.map(x => {
        let (len, body) = x
        box(width: len * width, height: height, stroke: 1pt + theme.main-color, align(center + horizon, {
...
- 链路层: 帧
  ...
  - WiFi(IEEE 802.11)
    #struc-table(width: 1.2em, height: 4em, info: [每行 32 字节], (
      (2, [帧\ 控制\ 2B]),
      (2, [持续\ 时间\ 2B]),
      (6, [地址1 6B]),
      (6, [地址2 6B]),
    ...
    其中帧控制部分有11个子字段
    #struc-table(width: 2.4em, height: 3em, info: [每行 16 位], (
      (2, [协议版本]),
      (2, [类型]),
      (4, [子类型]),
      (1, [去往\ DS]),
...
- 传输层
  ...
  - UDP: 数据报
    #struc-table(width: 4em, info: [每行 8 字节], (
      (2, [源端口号]),
      (2, [目的端口号]),
      (2, [总长度]),
      (2, [数据报校验和]),
